...
ctch$compute_properties() ## TODO - fix warning
...
tmp <- terra::ifel(tmp<=500,-998,-999)

ctch$add_layer(tmp, "greater_500")
ctch$get_layer()

tmp <- ctch$classify("atb_20","atb",cuts=20)
ctch$add_layer( tmp, "atb_20")
...
terra::plot(ctch$combine_classes("atb_20_band_500",pairs=c("atb_20","band"),burns="greater_500"))## TODO not working since burns are always full matrices of numbers
